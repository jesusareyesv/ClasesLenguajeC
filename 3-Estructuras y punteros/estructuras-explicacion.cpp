...
typedef struct{
	...
	float x,y;
}Beta;//As� le asignamos la etiqueta luego de la declaraci�n
...
// El unico problema de esta declaraci�n es que la hice por fuera del main y por lo tanto 'gerente' es una variable global (desaconsejado)
	"Carlos Mora",
	32,
	1.70,
	86.567
...
// Es parecido a los metodos de la programaci�n orientada a objetos
	float radio;

	float perimetro(){
		//podemos invocar a la variable radio desde aqu� porque dentro de la estructura est� definida como variable global para sus funciones internas
...
int main(void){
	...
	//punto_1 = 4; // definicion err�nea, porque la variable es del tipo Alpha, no es un entero, por lo tanto no podemos igualarlas

	// definici�n correcta (usando llaves {} ), asignamos un valor para cada uno de los enteros que se encuentran dentro de la estructura
	punto_1 = {4,6};

	printf("P1.a = %d y P1.b = %d",punto_1.a,punto_1.b);
	// Comprobamos que cargamos correctamente las variables
	// Como podemos ver, las variables dentro de la estructura se usan igual que cualquier otra del tipo int, float... s�lo que se encuentran
	// anidadas dentro de otro identificador (nombre) que pertenece a un tipo estructura definido por el programador

	// Seguimos jugando un poco con la asignaci�n de valores para comprobar que en efecto estas variables son como cualquier otra.
	...
	// Imprimimos el nombre del gerente que declaramos como variable global antes del main, en tiempo de ejecuci�n podemos ver que funciona perfectamente ese tipo de carga.
	printf("\n%s\n",gerente.nombre);
