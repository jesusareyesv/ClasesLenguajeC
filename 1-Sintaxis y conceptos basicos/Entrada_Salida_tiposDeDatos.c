...
int main(){
  ...
  long long entero_Largo; // Entero largo de 4 bytes (tamano constante en la arquitectura, a diferencia del int)

  /* Se pueden dar valores (definir) directamente en la declaracion*/
  float real=987.2;  //Real 4 bytes
  double real_Largo=6875.3123; //Real de 8 bytes

  char caracter;//1 byte
  char cadena1[20];//20 variables de tipo char (1 byte)
  char cadena2[tamano_cadena];


  printf(GREEN);
  unsigned int entero_sinSigno; //entero sin signo de 4 bytes
  //ASIGNACION
  ...
  printf("\nEntero largo = %d Tamano = %d",entero_Largo,sizeof(entero_Largo));
  //printf("Entero largo = %");
  ...
  printf(BLUE);
  //el .x antes de los caracteres de control de los tipos %f son para indicar la cantidad de decimales que se quieren mostrar, (X e Z)
